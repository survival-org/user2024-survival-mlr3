...
lrn_cvcoxboost = lrn("surv.cv_coxboost", penalty = "optimCoxBoostPenalty", iter.max = 25)
```
...
A visual approach is the critical difference plot, which shows a connecting line between the learners that _are not_ statistically different from each other (as far as their average ranks are concerned).
